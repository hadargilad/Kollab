    ...
    public void InitializeDatabase()
{
    using (var connection = new SqliteConnection(connectionString))
    {
        ...
        // 4. Table for Relationships
        string relationshipsTable = @"
        CREATE TABLE IF NOT EXISTS Relationships (
            Id INTEGER PRIMARY KEY AUTOINCREMENT,
            Speaker1Id INTEGER NOT NULL,
            Speaker2Id INTEGER NOT NULL,
            RelationshipType TEXT DEFAULT 'Direct',
            InteractionCount INTEGER DEFAULT 0,
            Strength REAL DEFAULT 0,
            Notes TEXT,
            FOREIGN KEY(Speaker1Id) REFERENCES Speakers(Id),
            FOREIGN KEY(Speaker2Id) REFERENCES Speakers(Id),
            UNIQUE(Speaker1Id, Speaker2Id)
        );";
        ...
        using (var cmd4 = new SqliteCommand(relationshipsTable, connection)) { cmd4.ExecuteNonQuery(); }
        using (var cmd5 = new SqliteCommand(userTable, connection)) { cmd5.ExecuteNonQuery(); }
    }
}
